day=int(input('date:'))  //вводим данные
month=int(input('month:'))
year_int=int(input('year'))
year=string(year_int)      //превращаем год в строку для того, чтобы сделать вырезку
n=length(year)
...
if month==or(01,10) then monthcode=1
elseif month==05 then monthcode=2
elseif month==08 then monthcode=3
elseif month==or(02,03,11) then monthcode=4
elseif month==06 then monthcode=5
elseif month==or(12,09) then monthcode=6
elseif month==or(04,07) then monthcode=0
    
end

//if modulo(year_int,4)==0 then ndy=366
//      else ndy=365
//end

//окончательная формула расчета дня недели
wd=day+monthcode+yearcode
